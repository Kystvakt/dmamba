...
class GradLoss(nn.Module):
    def __init__(self, *, t_dim, y_dim, x_dim, edge_idx, device):
        super().__init__()

        num_nodes = t_dim * y_dim * x_dim

        coords = torch.rand(1, num_nodes, 3, device=device)
        out_channels = torch.rand(num_nodes, 1, device=device)
        label = torch.rand(t_dim, y_dim, x_dim, device=device)
