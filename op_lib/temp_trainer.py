...
class TempTrainer:
    ...
    def train(self, max_epochs, log_dir, dataset_name):
        ...
        for epoch in range(max_epochs):
            train_loss, train_mse_loss = self.train_step(epoch)
            val_loss = self.val_step(epoch)

            if self.verbose:
                print((
                    f"Epoch {epoch} | "
                    f"train loss: {train_loss:.4f}, train mse: {train_mse_loss:.4f}, "
                    f"val loss: {val_loss:.4f}"
                ))

            # test each epoch
            test_dataset = self.val_dataloader.dataset.datasets[0]
            metrics = self.test(test_dataset)
    ...
    def train_step(self, epoch):
        ...
        epoch_loss = 0
        epoch_mse_loss = 0
        for iter_, (coords, temp, vel, label) in enumerate(self.train_dataloader):
            ...
            global_iter = epoch * len(self.train_dataloader) + iter_
            write_metrics(pred, label, global_iter, 'Train', self.writer)
            del temp, vel, label

        epoch_loss /= len(self.train_dataloader)
        epoch_mse_loss /= len(self.train_dataloader)
        if self.log_to_wandb:
    ...
    def val_step(self, epoch):
        val_epoch_loss = 0
        self.model.eval()
        for iter_, (coords, temp, vel, label) in enumerate(self.val_dataloader):
            coords = coords.to(self.device).float()
            temp = temp.to(self.device).float()
            vel = vel.to(self.device).float()
            label = label.to(self.device).float()
            with torch.no_grad():
                ...
                loss = temp_loss
            val_epoch_loss += loss
            global_iter = epoch * len(self.val_dataloader) + iter_
    ...
    def test(self, dataset, max_timestep=200):
        if is_leader_process():
            ...
            if self.verbose:
                # elapsed time
                print(f"Rollout time: {dur:.3f} (s)")
                print(metrics)

            exp_name = f'{self.model.__class__.__name__}'
            plt_temp(temps, labels, exp_name)
            plt_iter_mae(temps, labels, 'temperature', exp_name)
            dataset.reset()
