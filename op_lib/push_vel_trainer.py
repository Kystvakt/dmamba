...
class PushVelTrainer:
    ...
    def train_step(self, epoch, max_epochs):
        ...
        print(
            f'Epoch : {epoch} | train loss: {epoch_loss / len(self.train_dataloader):.6f}, temp loss: {epoch_temp_loss / len(self.train_dataloader):.6f}, vel loss: {epoch_vel_loss / len(self.train_dataloader):.6f}, time: {end - start:.2f}s')
        if self.log_to_wandb:
    ...
    def val_step(self, epoch):
        ...
        print(
            f'Epoch : {epoch} | val loss: {val_epoch_loss / len(self.val_dataloader):.6f} | val temp loss: {val_temp_loss / len(self.val_dataloader):.6f} | val vel loss: {val_vel_loss / len(self.val_dataloader):.6f}, time: {end - start :.2f}s')
        if self.log_to_wandb:
