...
class DiskTempInputDataset(DiskHDF5Dataset):
    ...
    def __init__(self, filename, steady_time, use_coords, transform=False, time_window=1, future_window=1,
                 push_forward_steps=1):
    ...
    def __getitem__(self, timestep):
        ...
        vel = torch.cat([self._get_vel_stack(timestep + k) for k in range(self.time_window + self.future_window)],
                        dim=0)
...
class DiskTempVelDataset(DiskHDF5Dataset):
    ...
    def __getitem__(self, timestep):
        ...
        """
        args = list(
            zip(*[self._get_timestep(timestep + k * self.future_window) for k in range(self.push_forward_steps)]))
